...
const seedDepartments = async () => {
  try {
    ...
    // Now assign evaluators to departments
    if (evaluators.length > 0) {
      const evaluatorsPerDept = Math.ceil(evaluators.length / departments.length);

      // Loop through each department and assign evaluators
      for (let i = 0; i < departments.length; i++) {
        // Slice the evaluators array to assign a subset to the current department
        const assignedEvaluators = evaluators.slice(i * evaluatorsPerDept, (i + 1) * evaluatorsPerDept);

        // Update the department to include the assigned evaluators
        await Department.findByIdAndUpdate(departments[i]._id, {
          $push: { evaluators: { $each: assignedEvaluators.map(evaluators => evaluators._id) } },
        });

        // Update the evaluators to include the department reference
        for (let evaluators of assignedEvaluators) {
          await User.findByIdAndUpdate(evaluators._id, {
            $set: { department: departments[i]._id },
          });
        }
      }
    ...
    // Now assign students to departments
    if (students.length > 0) {
      const studentsPerDept = Math.ceil(students.length / departments.length);

      // Loop through each department and assign students
      for (let i = 0; i < departments.length; i++) {
        // Slice the students array to assign a subset to the current department
        const assignedStudents = students.slice(i * studentsPerDept, (i + 1) * studentsPerDept);

        // Update the department to include the assigned students
        await Department.findByIdAndUpdate(departments[i]._id, {
          $push: { students: { $each: assignedStudents.map(students => students._id) } },
        });

        // Update the students to include the department reference
        for (let students of assignedStudents) {
          await User.findByIdAndUpdate(students._id, {
            $set: { department: departments[i]._id },
          });
        }
      }

      console.log("Students have been assigned to their respective departments.");
    } else {
